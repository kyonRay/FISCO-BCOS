 ...
 * @brief buildGenesisBlock L2 branch: allocs flat-KV writes + immutable hash row.
 */
...
BOOST_AUTO_TEST_CASE(L2BranchWritesAllocsToFlatKV)
{
    task::syncWait([this]() -> task::Task<void> {
        ...
        // 3) genesis_immutable_hash present in SYS_CURRENT_STATE
        auto immutableHashEntry = co_await storage2::readOne(*storage,
            executor_v1::StateKeyView(ledger::SYS_CURRENT_STATE, "genesis_immutable_hash"));
        BOOST_REQUIRE(immutableHashEntry);
        BOOST_CHECK_EQUAL(
            immutableHashEntry->get(), ledger::computeGenesisImmutableHash(genesisConfig).hex());
...
BOOST_AUTO_TEST_CASE(PbftBranchUnchanged)
{
    task::syncWait([this]() -> task::Task<void> {
        ...
        genesisConfig.m_compatibilityVersion =
            static_cast<uint32_t>(bcos::protocol::BlockVersion::V3_6_VERSION);
        genesisConfig.m_chainMode = "pbft";
        // no allocs

        auto ok = co_await ledger::buildGenesisBlock(*ledger, genesisConfig, param);
        BOOST_CHECK(ok);

        // pbft mode must NOT write a genesis_immutable_hash row
        auto immutableHashEntry = co_await storage2::readOne(*storage,
            executor_v1::StateKeyView(ledger::SYS_CURRENT_STATE, "genesis_immutable_hash"));
        BOOST_CHECK(!immutableHashEntry);

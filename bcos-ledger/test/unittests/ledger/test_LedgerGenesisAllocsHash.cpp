 ...
 * @brief Second-startup guard over genesis_immutable_hash. "Restart" is simulated
 *        by calling buildGenesisBlock a second time on the same StateStorage: the
 *        second call sees the existing genesis block and enters the verify path.
 */
...
BOOST_AUTO_TEST_CASE(SecondStartupChangedChainModeAborts)
{
    task::syncWait([this]() -> task::Task<void> {
        ...
        // flip chain_mode to pbft but keep allocs so the guard (allocs non-empty)
        // still fires. NodeConfig would reject this upstream; Ledger defends anyway.
        auto changed = config;
        changed.m_chainMode = "pbft";

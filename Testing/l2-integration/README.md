...
- The A8 devnet at `Testing/l2-devnet/docker-compose.yml` (NOT in this repo
...
bash Testing/l2-integration/run-all.sh

# Run one scenario:
bash Testing/l2-integration/genesis-bootstrap.sh
```
...
| `DEVNET_COMPOSE` | `<repo>/Testing/l2-devnet/docker-compose.yml` | devnet presence gate |
| `L2_PROXY_ADMIN_OWNER` | `0x000...000` | expected ProxyAdmin owner |
| `POLL_ATTEMPTS` / `MIN_ADVANCE` | `30` / `3` | gate-g3 block-advance poll |
| `L1_BRIDGE_TOOLING` | `<repo>/Testing/l2-devnet/l1-bridge/deploy.json` | gate-g4 A8 marker |

## SKIP semantics

There is no devnet in this repo (`Testing/l2-devnet/` is an A8-workstream
deliverable). Every scenario calls `require_devnet`: if the compose file is
absent it prints `SKIP: requires A8 devnet (Testing/l2-devnet)` and exits with

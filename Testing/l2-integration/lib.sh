...
# Repo root = three levels up from this file (Testing/l2-integration/lib.sh).
_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${_LIB_DIR}/../.." && pwd)"

# The A8 devnet compose file. Its presence is the single gate for every
# scenario that needs a live chain.
DEVNET_COMPOSE="${DEVNET_COMPOSE:-${REPO_ROOT}/Testing/l2-devnet/docker-compose.yml}"
...
require_devnet() {
    if [[ ! -f "${DEVNET_COMPOSE}" ]]; then
        skip "requires A8 devnet (Testing/l2-devnet)"
    fi
}

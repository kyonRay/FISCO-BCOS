 ...
 *        dropped disabledInL2() at any of the 13 EVM insert sites is invisible to
...
// SHARDING -> feature_sharding, BALANCE -> feature_balance_precompiled; the AUTH_*
// and ACCOUNT* gates are version-driven and pass at MAX_VERSION; PAILLIER is NOT in
// kL2DisabledSet so its feature is irrelevant here.
...
BOOST_AUTO_TEST_CASE(EvmProductionMapHidesAllL2DisabledMembers)
{
    ...
    // flag must not hide it.
    BOOST_CHECK(map->at(CRYPTO_ADDRESS, kMaxVersion, isAuth, l2) != nullptr);
    // CAST_ADDRESS is gated only by version (>= V3_2), also unaffected by L2.
    BOOST_CHECK(map->at(CAST_ADDRESS, kMaxVersion, isAuth, l2) != nullptr);
}
